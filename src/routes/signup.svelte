...
<main on:scroll={handleScroll} class="h-screen overflow-y-auto">
  <form on:submit|preventDefault={handleSubmitBtn}>
        <div class="container px-5 py-5 mx-auto flex flex-wrap justify-center">
          <div class="lg:w-2/6 md:w-1/2 h-fit bg-gray-100 rounded-lg p-8 flex flex-col w-full mt-10 md:mt-0">
            <h2 class="text-gray-900 text-lg font-bold title-font ">Sign Up</h2>
            <p class="mb-5 text-gray-900">It's quick and easy.</p>
            ...
            <button type="submit" value="Login" on:click={()=>changeState("LOGIN")} class="flex mx-auto ml-0 text-white bg-blue-500 border-0 py-2 px-8 focus:outline-none hover:bg-blue-600 rounded text-lg">Login</button>
          </div>
        </div>
  </form>
</main>
